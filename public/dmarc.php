...
function tmpl_page ($body, $reportid, $host_lookup = 1, $sort_order, $dom_select, $domains = array(),$cssfile, $org_select, $orgs = array(), $per_select, $periods = array() ) {
  ...
  # footter
  #--------------------------------------------------------------------------
    $html[] = "  <div class='footer'>Brought to you by <a href='http://www.techsneeze.com'>TechSneeze.com</a> - <a href='mailto:dave@techsneeze.com'>dave@techsneeze.com</a>";
	$html[] = "  <br>Modified, extended and adjusted for use with PostfixAdmin by GermanCoding</div>";
    //$html[] = "  </body>";
    //$html[] = "</html>";
...
// DISABLED - postfixadmin takes care of this.
// --------------------------------------------------------------------------
...
if( $dom_select <> '' ) {
  ...
  foreach ($domains as $domain) {
     if( $first ) {
        $where .= " WHERE (domain = '". $mysqli->real_escape_string($domain) . "'";
		$first = false;
	 } else {
        $where .= " OR domain = '" . $mysqli->real_escape_string($domain) . "'";
     }
  }
  $where .= " ) ";
}
...
if( $per_select <> '' ) {
  ...
  $where .= ( $where <> '' ? " AND" : " WHERE" ) . " year(mindate)= " . $mysqli->real_escape_string($ye) . " and month(mindate) = " . $mysqli->real_escape_string($mo);
